...
//---
 ...
 double SL = Bid-25*Point; //Define stop loss
 double TP = Ask+87.5*Point;  // define TP
 double SL_sell = close_price+25*SymbolInfoDouble(_Symbol,SYMBOL_POINT); 
 double TP_sell = close_price-87.5*SymbolInfoDouble(_Symbol,SYMBOL_POINT);
...
if(signal == "buy" && OrdersTotal()==0)
  {
  OrderSend(_Symbol,OP_BUY,lotSize,Bid,3,SL,TP,"Buy Taken",0,0,Red);
...
if(signal == "sell" && OrdersTotal()==0)
  ...
  //Hard stop based on 75% of account value.
  double accntBalance = AccountInfoDouble(ACCOUNT_BALANCE);
  double accntEquity = AccountInfoDouble(ACCOUNT_EQUITY);
  
  double equityPercentage = accntEquity/accntBalance*100;
