...
int StartHour = 12; // 8am NYC Start operation hour
int  LastHour = 20; // 4PM NYC Last operation hour
...
int OnInit()
  ...
  bool SendTelegramMessage( string url, string token, string chat, string text) {
   ...
   requestUrl = StringFormat( "%s/bot%s/sendmessage?chat_id=%s&text=%s&parse_mode=html", url, token, chat, text);
...
if(signal == "buy" && OrdersTotal()==0)
  ...
  string msg = "Buy order executed @ " + DoubleToStr(Ask,_Digits) + "\n" + 
...
if(signal == "sell" && OrdersTotal()==0)
  ...
  string msg = "Sell order executed @ " + DoubleToStr(Bid,_Digits) + "\n" + 
